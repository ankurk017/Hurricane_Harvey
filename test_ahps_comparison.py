...
wrf_pcp =     sum([getvar(Dataset(wrffile_out), "RAINC") + getvar(Dataset(wrffile_out), "RAINNC") for wrffile_out in wrfoutfile[-5:]])
...
cont = ax[0].contourf(ahps_lon, ahps_lat, ahps_pcp, levels=levels, locator=ticker.LogLocator(), cmap='jet')
...
cont = ax[1].contourf(wrf_lon, wrf_lat, wrf_pcp, cmap="jet", levels=levels, locator=ticker.LogLocator())
