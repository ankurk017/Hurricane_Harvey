...
wrfoutfile_pre = sorted(glob.glob(wrf_runs + "wrfout_d01*"))[::3]
...
model_ws = np.array([harvey['vmax'][np.where(harvey['date']==pd.to_datetime(str(val.values)).to_pydatetime())[0]] for val in obs_slp["Time"]], dtype='object')
model2_ws_error = (model_ws[np.array([ws.shape[0]!=0 for ws in model_ws])]-(obs_ws[np.array([ws.shape[0]!=0 for ws in model_ws])]*1.95).values).mean()

model_ws = np.array([harvey['vmax'][np.where(harvey['date']==pd.to_datetime(str(val.values)).to_pydatetime())[0]] for val in mslp_timeseries["Time"]], dtype='object')
model1_ws_error = (model_ws[np.array([ws.shape[0]!=0 for ws in model_ws])]-(ws_timeseries[np.array([ws.shape[0]!=0 for ws in model_ws])]*1.95).values).mean()

ws_error_var = f'WS Error: {model1_ws_error}, {model2_ws_error}'


model_ws = np.array([harvey['mslp'][np.where(harvey['date']==pd.to_datetime(str(val.values)).to_pydatetime())[0]] for val in obs_slp["Time"]], dtype='object')
model2_sp_error = (model_ws[np.array([ws.shape[0]!=0 for ws in model_ws])]-(obs_slp[np.array([ws.shape[0]!=0 for ws in model_ws])]).values).mean()

model_ws = np.array([harvey['mslp'][np.where(harvey['date']==pd.to_datetime(str(val.values)).to_pydatetime())[0]] for val in mslp_timeseries["Time"]], dtype='object')
model1_sp_error = (model_ws[np.array([ws.shape[0]!=0 for ws in model_ws])]-(mslp_timeseries[np.array([ws.shape[0]!=0 for ws in model_ws])]).values).mean()
