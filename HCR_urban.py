...
wrfoutfile = sorted(glob.glob(f'/nas/rstor/akumar/USA/PhD/Objective01/Hurricane_Harvey/WRF_Harvey_V1/{case}/WRF_9-3-51/WRFV4/wrfout_d02_2017-*27*'))
...
bb = np.array((wrf_lon.min().values, wrf_lon.max().values, wrf_lat.min().values, wrf_lat.max().values))
bb = [-100, -94.5, 26, 31]
...
for geometry in geometries:
    ax.add_geometries([geometry], ccrs.PlateCarree(), facecolor='none', edgecolor='blue')
