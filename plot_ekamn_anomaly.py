...
# mean
u_pre_mean, v_pre_mean, u_pst_mean, v_pst_mean, w_pre_mean, w_pst_mean = [], [] , [], [], [], []
for prefiles, postfiles in progressbar.progressbar(zip(pre_wrffiles, post_wrffiles)):
    ...
    u_pre_mean.append(wrf_assign_coords(getvar(ncfile_pre, "ua", units="m/s")))
    v_pre_mean.append(wrf_assign_coords(getvar(ncfile_pre, "va", units="m/s")))
    w_pre_mean.append(wrf_assign_coords(getvar(ncfile_pre, "wa", units="m/s")))
    ncfile_post = Dataset(postfiles)
    u_pst_mean.append(wrf_assign_coords(getvar(ncfile_post, "ua", units="m/s")))
    v_pst_mean.append(wrf_assign_coords(getvar(ncfile_post, "va", units="m/s")))
    w_pst_mean.append(wrf_assign_coords(getvar(ncfile_post, "wa", units="m/s")))

u_pre_clim = xr.concat(u_pre_mean, dim='Time').mean(dim='Time')
v_pre_clim = xr.concat(v_pre_mean, dim='Time').mean(dim='Time')
w_pre_clim = xr.concat(w_pre_mean, dim='Time').mean(dim='Time')

u_pst_clim = xr.concat(u_pst_mean, dim='Time').mean(dim='Time')
v_pst_clim = xr.concat(v_pst_mean, dim='Time').mean(dim='Time')
w_pst_clim = xr.concat(w_pst_mean, dim='Time').mean(dim='Time')

for prefiles, postfiles in progressbar.progressbar(zip(pre_wrffiles, post_wrffiles)):
    ncfile_pre = Dataset(prefiles)
    pres = getvar(ncfile_pre, "pres") / 100
    u_pre = wrf_assign_coords(getvar(ncfile_pre, "ua", units="m/s")) - u_pre_clim
    v_pre = wrf_assign_coords(getvar(ncfile_pre, "va", units="m/s")) - v_pre_clim
    w_pre = wrf_assign_coords(getvar(ncfile_pre, "wa", units="m/s")) - w_pre_clim

    ncfile_post = Dataset(postfiles)
    pres = getvar(ncfile_post, "pres") / 100
    u_post = wrf_assign_coords(getvar(ncfile_post, "ua", units="m/s")) - u_pst_clim
    v_post = wrf_assign_coords(getvar(ncfile_post, "va", units="m/s")) - v_pst_clim
    w_post = wrf_assign_coords(getvar(ncfile_post, "wa", units="m/s")) - w_pre_clim
    ...
    plt.tight_layout()
    plt.savefig(f"../figures/wind_profiles_anomaly/profiles_{str(u_pre.Time.values)[:13]}.jpeg")
    plt.close()
#    plt.show()
...
plt.tight_layout()
plt.savefig('../figures/wind_profiles_anomaly/region_vertical_wind_profiles.jpeg')
#plt.show()
