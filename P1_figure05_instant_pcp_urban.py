...
prefiles = sorted(glob.glob(home + f'/WRF_case01/WRF_plus3/test/em_real/wrfout_d01_2017-*'))[52:67]
postfiles = sorted(glob.glob(home + f'/WRF_case02/WRF_plus3/test/em_real/wrfout_d01_2017-*'))[52:72]


output_dir = f"../figures_paper/Ensemble/Rainfall/urban/"
...
for fileid in progressbar.progressbar(range(len(prefiles) - 1)):
    ...
    for pre_post_id in range(2):
        ...
        ax[pre_post_id].quiver(
            pre_post_uv[pre_post_id]["west_east"][::3],
            pre_post_uv[pre_post_id]["south_north"][::3],
            pre_post_uv[pre_post_id].sel(u_v="u")[::3, ::3],
            pre_post_uv[pre_post_id].sel(u_v="v")[::3, ::3],
            units="x",
            scale=40,
            width=0.022,
        )
