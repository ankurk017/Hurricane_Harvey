...
min_pressure_location = (lon[min_pressure_index[1]],
                         lat[min_pressure_index[0]])
# min_pressure_location = (lon[min_pressure_index[0]], lat[min_pressure_index[1]])
...
cont = plt.contourf(x, y, pressure)
# cont = plt.imshow(pressure, extent=[x.min(), x.max(), y.min(), y.max()])
...
uv_cs = uv_cs.assign_coords({'index': uv_cs['x']})
...
[ax.scatter(pressure_centroid[0], 1000, color='b') for ax in axs]
[ax.scatter(min_pressure_location[0], 1000, color='r') for ax in axs]
plt.tight_layout()
...
cont = plt.contourf(uv_cs['x'].values, uv_cs['level'].values, ws, cmap='jet')
...
ws = uv_cs['ua_interp']
fig, axs = plt.subplots(1, 1, figsize=(8, 7))
cont = plt.contourf(uv_cs['x'].values, uv_cs['level'].values, ws, cmap='bwr')
plt.scatter(pressure_centroid[0], 1000, color='b')
plt.scatter(min_pressure_location[0], 1000, color='r')
plt.gca().invert_yaxis()
plt.colorbar(cont)
