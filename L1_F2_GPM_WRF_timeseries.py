...
for wrf_files_id in progressbar.progressbar(range(len(wrfoutfile))):
    ...
    lat_id = np.where(np.logical_and(var_pre['south_north'].values>location[1]-0.1, var_pre['south_north'].values<=location[1]+0.1))[0]
    lon_id = np.where(np.logical_and(var_pre['west_east'].values>location[0]-0.1, var_pre['west_east'].values<=location[0]+0.1))[0]
...
#    )
    ...
    #    )
    #)
    var_timeseries_post.append(var_post.isel(south_north=lat_id, west_east=lon_id).mean())
...
gpm_rainfall_region = gpm.isel(lon=np.where(np.logical_and(gpm['lon']>location[0]-0.1, gpm['lon']<=location[0]+0.1))[0], lat=np.where(np.logical_and(gpm['lat']>location[1]-0.1, gpm['lat']<=location[1]+0.1))[0]).mean(dim='lon').mean(dim='lat')
...
plt.xlim((var_timeseries_post_merged["Time"][0], var_timeseries_post_merged["Time"][-1]))
plt.ylim((0, 510))
plt.tight_layout()
