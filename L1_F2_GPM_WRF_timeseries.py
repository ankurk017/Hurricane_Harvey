...
gpm_rainfall_region = gpm.isel(lon=np.where(np.logical_and(gpm['lon'] > location[0]-0.1, gpm['lon'] <= location[0]+0.1))[
                               0], lat=np.where(np.logical_and(gpm['lat'] > location[1]-0.1, gpm['lat'] <= location[1]+0.1))[0]).mean(dim='lon').mean(dim='lat')

#################

fig, axs = plt.subplots(1, 1, figsize=(7.5, 4.3))
# axs.plot(gpm_timestep, np.cumsum(gpm_rainfall.values), 'k-', label='GPM')
axs.plot(gpm_timestep, np.cumsum(
    gpm_rainfall_region.values), 'k-', label='GPM')
axs.plot(
