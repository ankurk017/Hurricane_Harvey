...
pre_files = home_2512 + f"/LULC_2001/WRFV4.5.2/test/em_real/"
...
start_date = f'2017-08-{dates[0]}'
end_date = f'2017-08-{dates[-1]}'

variable = 'precip'

pre_precip = get_precip(pre_files, dates=dates, var_name = variable, obs=harvey)[variable]
post_precip = get_precip(post_files, dates=dates, var_name = variable, obs=harvey)[variable]
...
pre_precip = get_precip(pre_files, dates=dates, var_name = variable, hurr_center_ref=True, obs=harvey)[variable]
post_precip = get_precip(post_files, dates=dates, var_name = variable, hurr_center_ref=True, obs=harvey)[variable]
...
gpm_polar = get_precip_gpm(cropped_gpm, var_name = 'precip', dates=dates, hurr_center_ref = True, obs=harvey)['precip'].mean(dim='time').isel(radius=np.arange(0, 30))*2 # convert half hourly to hourly
gpm_polar_mean = get_precip_gpm(cropped_gpm, var_name = 'precip', dates=dates, hurr_center_ref = True, obs=harvey)['precip'].mean(dim='angle')*2
...
plt.tight_layout()
plt.savefig('../../figures_draft02/extended_figure02_polar_plot.jpeg', dpi=400, bbox_inches='tight')
...
cbar.ax.set_ylabel('Precipitation Rate(mm/hr)')
plt.tight_layout()
plt.savefig('../../figures_draft02/extended_figure02_polar_temporal.jpeg', dpi=400, bbox_inches='tight')
