...
def calculate_lwp(filein):
    ...
    qc = numpy.array(fn.variables["QCLOUD"][:, :, :, :])
    # Water vapor mixing ratio [kg/kg]
    qv = numpy.array(fn.variables["QVAPOR"][:, :, :, :])
    pres = numpy.array(fn.variables["PB"][:, :, :, :])  # Pressure [Pa]
    # Potential Temperature [K]
    theta = numpy.array(fn.variables["T"][:, :, :, :]) + \
        numpy.array(fn.variables["T00"][0])
    # Height on model levels [m]
    zh = numpy.array(fn.variables["PHB"][:, :, :, :])
    fn.close()
    ...
    for k in range(nz-1):
        lwp += (rho[:, k, :, :]+rho[:, k+1, :, :])/2*(qc[:, k, :, :] +
                                                      qc[:, k+1, :, :])/2*(zh[:, k+1, :, :]-zh[:, k, :, :])
