...
def plot_track_intensity(harvey, track_details_array, labels, colors):
    ...
    axs[1].plot(harvey["time"], harvey["mslp"],
                color="k", label="OBS", marker='o')
    for track_details, labs, cols in zip(track_details_array, labels, colors):
        axs[1].plot(
...
def plot_track(harvey, track_details_array, labels, colors, extent=None, figsize=(9, 7), legend_fontsize=14):
    ...
    for index in np.arange(0, harvey.time.shape[0], 4):
        ax.text(hurdat_crop["lon"][index]+0.05, hurdat_crop["lat"][index], str(hurdat_crop["time"][index])[
                :13].replace(' ', 'T')[8:], transform=ccrs.PlateCarree(), fontsize=legend_fontsize, color='#7D7D7D')
    ...
    for out1a in out:
        ...
        mslp_error.append(
            (
                ...
                - (
                    out1a["mslp"][np.array(
                        [ws.shape[0] != 0 for ws in model_ws])]
                ).values
            ).mean()
        )
        ...
        track_error.append(
            np.sqrt((np.concatenate(model2_lon**2 + model2_lat**2)
                     ).astype('float')).mean() * 111.11
        )
