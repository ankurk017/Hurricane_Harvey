...
def plot_ahps(date, plot_axis, ahps_mean=None, ):
    ...
    plot_axis.set_title(f'(b) AHPS (201708{date})', fontsize=15)
    #plot_axis.text(-95.5, 26.0, f'{np.round(ahps_mean, 2)} mm',
    #                   verticalalignment='bottom', horizontalalignment='left', fontsize=14, color='blue')
...
def plot_accp(wrfoutfile, ahps_grids, plot_axis, year_label='2001'):
    ...
    cor = correlation_without_nans(
        ahps_pcp_domain.ravel(), wrf_pcp_domain.ravel())
    # title = f"{ahps_files[ahps_fileid].split('/')[-1].split('_')[-2]} | {case} | RMSE: {np.round(rms, 2)}, R: {np.round(cor, 2)}"
    plot_axis.set_title(
        f'{year_label} | {np.round(rms, 2)} / {np.round(cor, 2)}', fontsize=15)

    plot_axis.text(np.nanmax(wrf_lon)-1.5, np.nanmin(wrf_lat), f'{np.nanmean(wrf_pcp_domain).round(2)} mm',
                   verticalalignment='bottom', horizontalalignment='right', fontsize=15, color='blue')

    return cont, np.nanmean(ahps_pcp_domain)
...
def get_wrffiles(base_path): return (
    sorted(glob.glob(base_path + f'/wrfout_d02_2017-*{int(date)}*00')))


def get_allwrffiles(base_path): return (
    sorted(glob.glob(base_path + f'/wrfout_d02_*00')))


domain_bb = [-100, -93, 25.5, 31.5]
levels = np.array((10, 20, 30, 40, 50, 100, 150, 200,
                  250, 300, 400, 500, 600, 700, 800, 900))

date = '27'

wrfoutfile_2001 = get_wrffiles(
    '/nas/rgroup/stela/akumar/WRF_Harvey_v4/WRF_Simulations_FNL/LULC_2001/WRFV4.5.2/test/em_real/')
wrfoutfile_2017 = get_wrffiles(
    '/nas/rgroup/stela/akumar/WRF_Harvey_v4/WRF_Simulations_FNL/LULC_2017/WRFV4.5.2/test/em_real/')

wrfoutfile_2050 = get_wrffiles(
    '/nas/rstor/akumar/USA/PhD/Objective01/Hurricane_Harvey/WRF_Harvey_V3/WRF_simulations//WRF_FNL_2050/WRF/test/em_real/')
wrfoutfile_2100 = get_wrffiles(
    '/nas/rstor/akumar/USA/PhD/Objective01/Hurricane_Harvey/WRF_Harvey_V3/WRF_simulations//WRF_FNL_2100/WRF/test/em_real/')


fig = plt.figure(figsize=(15.5, 9))
gs = gridspec.GridSpec(2, 4, width_ratios=[
                       1, 1, 1, 0.05], wspace=0.4, left=0.07, right=0.95)

ahps_grid_ax = plt.subplot(gs[0, 1], projection=ccrs.PlateCarree())
cont_2001_ax = plt.subplot(gs[0, 2], projection=ccrs.PlateCarree())
cont_2017_ax = plt.subplot(gs[1, 0], projection=ccrs.PlateCarree())
cont_2050_ax = plt.subplot(gs[1, 1], projection=ccrs.PlateCarree())
cont_2100_ax = plt.subplot(gs[1, 2], projection=ccrs.PlateCarree())
colorbar_ax = plt.subplot(gs[:, -1])

ahps_grid = plot_ahps(date=date, plot_axis=ahps_grid_ax)
plot_accp(wrfoutfile_2001, ahps_grid,
          plot_axis=cont_2001_ax, year_label='(c) LULC 2001')
plot_accp(wrfoutfile_2017, ahps_grid,
          plot_axis=cont_2017_ax, year_label='(d) LULC 2017')

plot_accp(wrfoutfile_2050, ahps_grid,
          plot_axis=cont_2050_ax, year_label='(e) LULC 2050')
cont, ahps_mean = plot_accp(
    wrfoutfile_2100, ahps_grid, plot_axis=cont_2100_ax, year_label='(f) LULC 2100')

ahps_grid_ax.text(-95., 26.0, f'{np.round(ahps_mean, 2)} mm',
          verticalalignment='bottom', horizontalalignment='left', fontsize=15, color='blue')

cbar = plt.colorbar(cont, cax=colorbar_ax, ticks=levels)
cbar.ax.set_yticklabels(levels)
cbar.ax.set_ylabel('Precipitation (mm)')
colorbar_ax.axes.set_position([0.92, 0.15, 0.02, 0.7])


plt.tight_layout()
plt.savefig('../../figures_draft02/FNL_fig02a.jpeg',
...
wrfoutfile_2001 = get_allwrffiles(
    '/nas/rgroup/stela/akumar/WRF_Harvey_v4/WRF_Simulations_FNL/LULC_2001/WRFV4.5.2/test/em_real/')[:90]
wrfoutfile_2017 = get_allwrffiles(
    '/nas/rgroup/stela/akumar/WRF_Harvey_v4/WRF_Simulations_FNL/LULC_2017/WRFV4.5.2/test/em_real/')[:90]

wrfoutfile_2050 = get_allwrffiles('/nas/rstor/akumar/USA/PhD/Objective01/Hurricane_Harvey/WRF_Harvey_V3/WRF_simulations//WRF_FNL_2050/WRF/test/em_real/')
wrfoutfile_2100 = get_allwrffiles('/nas/rstor/akumar/USA/PhD/Objective01/Hurricane_Harvey/WRF_Harvey_V3/WRF_simulations//WRF_FNL_2100/WRF/test/em_real/')
...
plt.savefig('../../figures_draft02/fig02b.jpeg', dpi=400)
