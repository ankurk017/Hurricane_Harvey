...
obs_slp  = xr.concat(
    [ getvar(ncfile, "slp", timeidx=timeid).min() for timeid in np.arange(0, total_time_ids, 1)],
    dim="Time",
)
obs_ws  = xr.concat(
    [getvar(ncfile, "uvmet10_wspd_wdir", timeidx=timeid).sel(wspd_wdir='wspd').max() for timeid in np.arange(0, total_time_ids, 1)],
    dim="Time",
)
