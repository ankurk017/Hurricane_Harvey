...
wrfoutfile_pre = sorted(
    glob.glob(home_2512 + f"/pre/WRF/WRF/test/em_real/wrfout_d02_20*")
)[13:43]
wrfoutfile_post = sorted(
    glob.glob(home_2512 + f"/post/WRF/WRF/test/em_real/wrfout_d02_20*")
)[13:43]


prefiles = wrfoutfile_pre
postfiles = wrfoutfile_post

output_dir = f"../figures/Laura/Ensemble/Rainfall/{case[1:]}/"
image_files = []
years = (
    2001,
    2017,
...
with imageio.get_writer(gif_file, mode="I", duration=6) as writer:
