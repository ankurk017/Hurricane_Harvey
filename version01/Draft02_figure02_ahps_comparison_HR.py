...
def plot_ahps(date, plot_axis):
	...
	plot_axis.set_title(f'(b) AHPS (201708{date})', fontsize=13.5)
	#plot_axis.text(-95.5, 26.0, f'{np.round(np.nanmean(ahps_pcp), 2)} mm', 
...
get_wrffiles_d01 = lambda base_path: (sorted(glob.glob(base_path + f'/wrfout_d01_2017-*{int(date)-1}*')) + sorted(glob.glob(base_path + f'/wrfout_d01_2017-*{int(date)}*')))[12:12+25]
...
wrfoutfile_2001 = get_wrffiles('/nas/rgroup/stela/akumar/WRF_Harvey_v4/WRF_Simulations_FNL/LULC_2001/WRFV4.5.2/test/em_real/')
wrfoutfile_2017 = get_wrffiles('/nas/rgroup/stela/akumar/WRF_Harvey_v4/WRF_Simulations_FNL/LULC_2017/WRFV4.5.2/test/em_real/')
...
plot_accp(wrfoutfile_2001, ahps_grid, plot_axis=cont_2001_ax, year_label='(c) LULC 2001')
plot_accp(wrfoutfile_2017, ahps_grid, plot_axis=cont_2017_ax, year_label='(d) LULC 2017')

plot_accp(wrfoutfile_2050, ahps_grid, plot_axis=cont_2050_ax, year_label='(e) LULC 2050')
cont = plot_accp(wrfoutfile_2100, ahps_grid, plot_axis=cont_2100_ax, year_label='(f) LULC 2100')
...
plt.tight_layout()
plt.savefig('../figures_draft01/FNL_fig02a.jpeg', dpi=400, bbox_inches='tight')
plt.show()
...
plt.savefig('../figures_draft01/fig02b.jpeg', dpi=400)
