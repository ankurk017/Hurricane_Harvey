...
wrfoutfile_pre = sorted(glob.glob(home_2512 + f'/pre_UCM/WRF{case}/test/em_real/wrfout_d02_2017-*'))[19:25]
wrfoutfile_post = sorted(glob.glob(home_2512 + f'/post_UCM/WRF{case}/test/em_real/wrfout_d02_2017-*'))[19:25]
...
def get_uv(prenc,  postnc):
 ...
 pre_v_cross = vertcross(getvar(prenc, "wa", units="m/s"), pres, levels=np.arange(1000, 700, -5), wrfin=prenc, start_point=start_point, end_point=end_point, latlon=True, meta=True,)
 ...
 post_v_cross = vertcross(getvar(postnc, "wa", units="m/s"), pres, levels=np.arange(1000, 700, -5), wrfin=postnc, start_point=start_point, end_point=end_point, latlon=True, meta=True,)
...
plt.tight_layout()
plt.savefig('../figures/cross_section_cwlp_uw_actual_UCM_ensemble/cross.jpeg')
#plt.show()
