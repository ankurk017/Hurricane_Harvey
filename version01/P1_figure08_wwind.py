...
#    divergence_pre = crop_region(wrf_assign_coords(getvar(Dataset(prefiles[fileid + 1]), "pvo")), location=urban_change)
#    divergence_post =  crop_region(wrf_assign_coords(getvar(Dataset(postfiles[fileid + 1]), "pvo")), location=urban_change)

    divergence_pre = crop_region(wrf_assign_coords(interplevel(getvar(Dataset(prefiles[fileid + 1]), "pvo"), wrf_pcp_pre_pres, 750)), location=urban_change)
    divergence_post = crop_region(wrf_assign_coords(interplevel(getvar(Dataset(postfiles[fileid + 1]), "pvo"), wrf_pcp_pre_pres, 750)), location=urban_change)
...
#    plt.show()
    ...
    plt.savefig(f"../figures_paper/pvo/Houston_divergence_{str(slp.Time.values)[:13]}.jpeg", dpi=400)
    plt.close()
