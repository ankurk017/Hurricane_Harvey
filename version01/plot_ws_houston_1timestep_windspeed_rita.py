...
urban_change = {"south_north": 29.74, "west_east": -95.49, "box": 4.5} # have to change later
...
wrfoutfile_pre = sorted(
    glob.glob(home_2512 + f"/pre/WRF{case}/test/em_real/wrfout_d01_20*")
)[30:45]#[24:40]
wrfoutfile_post = sorted(
    glob.glob(home_2512 + f"/post/WRF{case}/test/em_real/wrfout_d01_20*")
)[30:45]#[24:40]
...
output_dir = f"../figures/Rita/Ensemble/Rainfall/{case[1:]}/"
image_files = []
years = (
    2001,
    2017,
    ...
    for pre_post_id in range(2):
       ...
       #     locator=ticker.LogLocator(),
        ...
        ax[pre_post_id].quiver(
            pre_post_uv[pre_post_id]["west_east"][::7],
            pre_post_uv[pre_post_id]["south_north"][::7],
            pre_post_uv[pre_post_id].sel(u_v="u")[::7, ::7] ,
            pre_post_uv[pre_post_id].sel(u_v="v")[::7, ::7],  units='x', scale=40, width=0.032, 
        )
