...
wrfoutfile_pre = sorted(glob.glob(home_2512 + f"/LULC_2001/WRFV4.5.2/test/em_real/wrfout_d02_2017-*"))[ :65]
wrfoutfile_post = sorted(glob.glob(home_2512 + f"/LULC_2017/WRFV4.5.2/test/em_real/wrfout_d02_2017-*"))[ :65]
index = np.min((len(wrfoutfile_pre), len(wrfoutfile_post)))

prefiles = wrfoutfile_pre[:index]
postfiles = wrfoutfile_post[:index]

output_dir = f"../figures_draft01/Rainfall/"
...
for fileid in progressbar.progressbar(range(len(prefiles) - 1)):
    ...
    for pre_post_id in range(2):
        ...
        ax[pre_post_id].quiver(
            pre_post_uv[pre_post_id]["west_east"][::skip],
            pre_post_uv[pre_post_id]["south_north"][::skip],
            pre_post_uv[pre_post_id].sel(u_v="u")[::skip, ::skip],
            pre_post_uv[pre_post_id].sel(u_v="v")[::skip, ::skip],
            units="x",
            scale=40,
            width=0.022,
        )
        coast.plot_coast(ax[pre_post_id], houston=True)
    ...
    coast.plot_coast(ax[2], houston=True)
