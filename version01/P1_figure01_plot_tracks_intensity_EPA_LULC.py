...
wrf_pbl00 = sorted(glob.glob(home + f'/WRF_FNL_post/WRF/test/em_real/wrfout_d02_2017-*'))[::2]
wrf_pbl01 = sorted(glob.glob(home + f'/WRF_FNL_2050/WRF/test/em_real/wrfout_d02_2017-*'))[::2]
wrf_pbl02 = sorted(glob.glob(home + f'/WRF_FNL_2100/WRF/test/em_real/wrfout_d02_2017-*'))[::2]
...
plt.savefig('../figures_paper/EPA/pbl_track_pre_post.jpeg', dpi=400)
...
plt.savefig('../figures_paper/EPA/pbl_intensity_pre_post.jpeg', dpi=400)
...
pd.DataFrame(src.wrf_track.calculate_error(out, harvey), index=lab[:len(out)]).to_csv(f'../figures_paper/EPA/{titles}_track_stats.csv')
