...
# cross section 1
#start_point = CoordPair(lat=30.51, lon=-96.12)
#end_point = CoordPair(lat=29.15, lon=-94.86)
...
for prefiles, postfiles in progressbar.progressbar(zip(pre_wrffiles, post_wrffiles)):
    ...
    plt.tight_layout()
    plt.savefig(f"../figures/cross_section_w_uv_actual_UCM_ensemble/{case}_w_{str(wspd.Time.values)[:13]}.jpeg")
    plt.close()
#    plt.show()
...
plt.tight_layout()
plt.savefig('../figures/cross_section_w_uv_actual_UCM_ensemble/cross.jpeg')
#plt.show()
