...
def get_radial_tangential_wind(infile, pres_levels=np.arange(1000, 100, -10), radial_extent=5
):
...
wrf_files_pre = sorted(glob.glob('/nas/rgroup/stela/akumar/WRF_Harvey_v4/WRF_Simulations_FNL/LULC_2001/WRFV4.5.2/test/em_real/wrfout_d02_2017-08-26*'))
wrf_files_post = sorted(glob.glob('/nas/rgroup/stela/akumar/WRF_Harvey_v4/WRF_Simulations_FNL/LULC_2017/WRFV4.5.2/test/em_real/wrfout_d02_2017-08-26*'))


outdir = '../figures_draft01/radial_tangential_1000-800_FNL/'

for index in range(len(wrf_files_post)):
 print(index)
 prefix = wrf_files_post[index].split('/')[-1][11:24]

 rad_tang_pre = get_radial_tangential_wind(wrf_files_pre[index], pres_levels=np.arange(1000, 790, -10))
 rad_tang_post = get_radial_tangential_wind(wrf_files_post[index], pres_levels=np.arange(1000, 790, -10))
...
axs.plot(hr_pre[0], hr_pre[0].level, 'b', label='LULC 2001')
axs.plot(hr_post[0], hr_post[0].level, 'r', label='LULC 2017')
