...
for index in range(len(harvey['lon'])):
	print(index)
	d2bb = harvey['lon'][index]-3, harvey['lon'][index]+3, harvey['lat'][index]-3, harvey['lat'][index]+3
	d3bb = harvey['lon'][index]-2, harvey['lon'][index]+2, harvey['lat'][index]-2, harvey['lat'][index]+2
	...
	scatter = plot_hurr_track(ax, year=2019, name='dorian')  # Landfall over Bahamas
	...
	plt.tight_layout()
	plt.savefig(f'../figures/animation/Animation_{str(index).zfill(2)}.jpeg')
	#plt.show()
	plt.close()
